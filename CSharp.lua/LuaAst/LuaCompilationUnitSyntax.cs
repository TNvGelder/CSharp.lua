...
namespace CSharpLua.LuaAst {
  ...
  public sealed class LuaCompilationUnitSyntax : LuaSyntaxNode {
    ...
    public bool IsRoblox { get; }
    public string SystemNamespace { get; }
    ...
    public LuaCompilationUnitSyntax(string filePath = "", bool hasGeneratedMark = true, bool isRoblox = false, string systemNamespace = "") {
      FilePath = filePath;
      IsRoblox = isRoblox;
      ...
      // In Roblox mode or with custom namespace, import from _G.<namespace>.System
      LuaExpressionSyntax systemValue;
      if (isRoblox || !string.IsNullOrEmpty(systemNamespace)) {
        systemValue = LuaIdentifierNameSyntax.CreateGlobalSystem(systemNamespace);
      } else {
        systemValue = system;
      }
      AddImport(system, systemValue);
    }
    ...
    private void CheckUsingDeclares() {
      ...
      } else if (importAreaStatements.Statements.Count > 0) {
        // For files without namespace (e.g., manifest.lua), insert imports after any leading comments
        int insertIndex = 0;
        while (insertIndex < Statements.Count && Statements[insertIndex] is LuaShortCommentStatement) {
          insertIndex++;
        }
